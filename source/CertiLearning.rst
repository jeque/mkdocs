...
要访问AWS云服务，可以使用 ``AWS管理控制台`` ， ``AWS命令行界面(CLI)`` 或 ``AWS软件开发工具包(SDK)`` 。
 ...
 + 定价选项，实例按每小时处于运行状态收费
	有以下三种定价选项：
     ...
     - 预留实例，可预订可预测的工作负载。可以节省高达75%的按需小时费率。
		有两个因素决定了预订的成本：
		 * 承诺期限，长期承诺时间越长、折扣越大。
		 * 付款选项，预留实例有三种不同的付款选项： ``所有预付款`` 、 ``部分预付`` 和 ``无需预付`` 。
     - 竞价型实例，对于时间要求不高且容忍中断的工作负载。竞价型实例可提供最大的折扣。AWS终止实例提供两分钟的警告。	 
 + 租赁选择
	实例有如下租户选项：
	 - 共享租赁:所有实例的默认租赁模式。单台主机可能容纳来自不同客户的实例。
	 - 专用实例：运行在专用于单个客户的硬件上。
	 - 专用主机：EC2专用主机是一台物理服务器，其实例容量完全专用于单个客户的使用。可解决许可需求。
 + 展示位置组
	是实例的单个可用区域内的逻辑分组。放置组使应用程序能够参与低延迟的10Gbps网络。

*实例商店*
############
一个实例存储为您的实例临时块级存储。该存储位于物理连接到主机的磁盘上。

实例商店包含在EC2实例的成本中， ``实例存储是实现适当工作负载的经济方法`` ，但持久性有限。实例商店的关键方面是它们是 `临时的` 。

实例存储的数据在以下情况下会丢失：
 
 + 底层磁盘驱动器失败
 + 实例停止（重新启动，数据持续存在），实例终止。
 
Amazon ELastic Block Store（Amazon EBS）
##########################################
基于实例存储的有限的持久性，Amazon提供更持久的块存储的工作负载方式-Amazon EBS。

*弹性块存储基础知识*
######################
Amazon EBS提供持久的块级存储卷以用于Amazon EC2实例，每个EBS卷都会在其可用区内自动复制，以防止组织发送故障。

多个EBS卷可以连接到单个EC2实例，但一次只能将卷连接到单个实例。

*Amazon EBS卷的类型*
#####################
磁性体积
^^^^^^^^^
所有亚马逊EBS卷类型的磁卷具有最低的性能特性。

亚马逊EBS磁性卷的大小范围是从1GB到1TB不等，平均为100个IOPS，但有能力突破数百IOPS。 ``最适合于不常访问数据的工作负载`` 

通用SSD
^^^^^^^^
容量范围可以从1GB到16TB不等，并提供每GB配置3个IOPS的基准性能，上限为10,000 IOPS。

通用SSD卷适用于磁盘性能不是最关键的广泛工作负载：
...
预置IOPS SSD
^^^^^^^^^^^^^
旨在 ``满足I/O密集型工作负载`` 的需求，尤其是对存储性能和随机访问I/O吞吐量一致性敏感的数据库工作负载。

预置IOPS SSD卷的大小范围可以从4GB到16TB。配置时，不仅要指定大小，还要指定所需的IOPS数量，最多为卷的GB书的30倍，或者20,000 IOPS。

预置IOPS SSD卷提供可预测的高性能，非常适合：
...
吞吐量优化HDD卷
^^^^^^^^^^^^^^^^
大小范围可达16TB，最大IOPS为500，最大吞吐量为500MB/s。比通用SSD卷要便宜的多。

低成本HDD卷，专为频繁访问、吞吐量密集型工作负载：
...
冷硬盘卷
^^^^^^^^^
专为较少访问的工作负载而设计，如较冷的数据。

卷可以高达16TB，最大IOPS为250，最大吞吐量为250MB/s，这些卷比吞吐量优化的HDD卷还要便宜。

Amazon EBS优化的实例
^^^^^^^^^^^^^^^^^^^^^^^
使用磁性和EBS I/O之外的任何卷类型，则使用Amazon EBS优化实例很重要。

*保护数据*
############
备份/恢复(快照)
^^^^^^^^^^^^^^^^^
快照是增量备份，意味着只保存最近一次快照以来更改过的块。
...
数据使用Amazon S3技术进行存储，因为是存储在AWS控制的存储中，不能像其他S3对象一样操作它们，而是必须使用Amazon EBS快照功能来管理。

快照还可以增加Amazon EBS卷的大小。

恢复卷
^^^^^^^^^^
EBS卷在实例的生命周期之外持续存在，因此如果实例出现故障，可以恢复数据。

加密选项
^^^^^^^^^^
EBS在所有卷类型上提供本机加密。

当启动加密的EBS卷时，Amazon会使用AWS秘钥管理服务(KMS)来处理秘钥管理。数据和相关秘钥使用业界标准的AES-256算法进行加密。
...
Amazon VPC介绍
################
是AWS云内的自动以的虚拟网络，即虚拟私有云。

Amazon VPC是Amazon EC2的联网层，它允许在AWS内构建自己的虚拟网络。可以控制VPC的各个方面：
 
 + 选择自己的IP地址范围 
 + 创建自己的子网
 + 配置自己的路由表、网关和安全设置

在一个区域内，可以创建多个VPC，并且每个VPC都是逻辑隔离的，即使它共享其IP地址空间。
...
结构化方法
^^^^^^^^^^^
结构化方法是属于自顶向下的开发方法，基本思想 ``自顶向下、逐步求精`` ，强调开发方法的结构合理性及所开发软件的结构合理性。

结构化方法包括了 ``结构化分析(Structured Analysis， SA)`` 、 ``结构化设计(Structured Design，SD)`` 和 ``结构化程序设计(Structured Programing， SP)`` 等方法。

+ 结构化分析(SA)：
 SA使用 ``抽象模型`` 的概念，按照软件内部数据传递、变换的关系，自顶向下、逐层分解，直至找到满足功能要求的所有可实现的软件为止。
 
 一般 ``利用图形`` 表达用户需求，使用的手段主要有 `数据流图` 、 `数据字典` 、 `结构化语言` 、 `判定表及判定树` 等。
+ 结构化设计(SD)：
...
净室软件工程
^^^^^^^^^^^^^
软件开发的一种 ``形式化方法`` ，可形成高质量的软件。使用 ``盒结构规约`` 进行分析和设计建模，并且强调 ``正确性验证`` 作为发现和消除错误的主要机制，使用 ``统计的测试`` 来获取认证被交付的可靠性所必需的出错率信息。

净室方法主要使用三张盒类型：
 
 + 黑盒，这种盒规划系统或系统的某部分的行为。
 + 状态盒，以类似于对象的方式封装状态数据和服务。
 + 清晰盒，定义状态盒所蕴含的变迁功能，简单说，清晰盒包含了对状态盒的过程设计。
...
需求分析：
 ...
 UML：由构造块、规则和公共机制
	...
	图：
		+ 结构图(静态图)：类图、对象图、包图、组合结构图、构件图、部署图
		+ 行为图(动态图)：用例图、交互图（包括顺序图、通信图、定时图）、状态图、状态转换图、活动图（可以表现并发）、交互概览图
		
		4+1视图：逻辑视图、实现视图、进程视图、部署视图 +用例视图
		
	建模：用例模型和分析模型
	
	用例图：用来描述一组用例、参与者及它们之间的关系
	用例模型的流程：
	...
	类图与对象图：
		...
		+ 依赖关系
		+ 泛化关系
		+ 关联关系-- 聚合关系（生命周期不通，如汽车和轮子）、组合关系（生命周期相同，如公司和部门）
		+ 实现关系：接口与类之间的关系
		
		箭头： ``菱形代表整体和部分，空心箭头泛化关系`` 
		
	顺序图：是一种交互图，强调对象之间消息发送的顺序，同时显示对象之间的交互。
	
	活动图：控制流和数据流   泳道图-活动图的一种
...
软件系统建模
^^^^^^^^^^^^^
逆向正向工程：现有系统--物理系统-->物理模型-->逻辑模型--> *逻辑模型-->物理模型-->物理系统--新系统*
...
设计原则：
 ...
 + 接口隔离原则：使用多个专门的接口比使用单一的要好
 + 组合重用原则：尽量使用组合而不是继承
 + 迪米特原则：最少知识法则，一个对象对其他对象有尽可能少的了解

设计模式的概念
 
 + 架构模式：软件中的高层决策
 + 设计模式：软件系统的设计，与具体的实现语言无关
 + 惯用法：最低层的模式
...
**设计模式的分类**
 ...
 + 行为型模式：更大的结构
	...
	- 观察者模式(Observer)：联动
	- 状态模式(State)：状态变成类
	- 策略模式(Strategy)：定义一系列算法，多方案切换
	- 模板方法模式(Template Method)：
	- 访问者模式(Visitor)：数据与操作分离
...
测试类型
 ...
 + 动态测试-计算机执行：黑盒测试、白盒测试、灰盒测试
 + 静态测试-人工检查：桌前检查、代码审查、代码走查
 
测试用例设计

 + 黑盒测试法：等价类划分、边界值分析、错误检测、因果图
 + 白盒测试法：
	- 基本路径测试
	- 循环覆盖测试
	- 逻辑覆盖测试
